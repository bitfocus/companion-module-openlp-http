...
class instance extends instance_skel {
  constructor(system, id, config) {
    ...
    this.choices_mode_with_toggle = [
      { id: "toggle", label: "Toggle Blank/Show" },
      ...this.choices_mode,
    ];
    ...
    this.system.emit(
      ...
      (err, result) => {
        if (err !== null) {
          ...
          this.log(
            "error",
            "HTTP GET Request failed (" + result.error.code + ")"
          );
        } else {
  ...
  initWebSocket = (websocket_port) => {
    ...
    if (!websocket_port) {
      this.status(
        this.STATUS_ERROR,
        "Configuration error - no WebSocket port defined"
      );
      return;
    }
  ...
  throw401Warning = () => {
    ...
    this.status(
      this.STATUS_WARNING,
      "Limited connection, only variables will work. Login is required."
    );
  };
  ...
  init_feedbacks = () => {
    const feedbacks = {
      ...
      fbk_si: {
        ...
        description:
          "If specific service item is active, change style of the bank",
        style: {
          color: this.rgb(255, 255, 255),
          bgcolor: this.rgb(255, 0, 0),
        },
  ...
  actionV2 = (action) => {
    ...
    switch (action.action) {
      case "gotoSi":
        urlAction =
          "service/set?data=" +
          JSON.stringify({ request: { id: Number(action.options.si - 1) } });
        break;
      case "refreshSiList":
      ...
      case "gotoSlide":
        urlAction =
          "controller/live/set?data=" +
          JSON.stringify({ request: { id: Number(action.options.slide - 1) } });
        break;
    }
    const url =
      "http://" + this.config.ip + ":" + this.config.port + "/api/" + urlAction;
    //console.log(url)
    this.system.emit(
      "rest_get",
      url,
      this.interpretActionResult,
      this.headersV2()
    );
  ...
  actionV3 = (action) => {
    ...
    const url =
      ...
      `://${this.config.ip}:${this.config.port}/api/v2/${urlAction}`;
  ...
  interpretActionResult = (err, result) => {
    if (err !== null) {
      ...
      if (result.response.statusCode == 401) {
        ...
        this.status(
          this.STATUS_ERROR,
          "Got status code " + result.response.statusCode + " from OpenLP."
        );
      } else {
        this.auth_error = false;
        this.status(this.STATUS_OK);
  ...
  interpretPollData = (data) => {
    ...
    if (
      data.service > this.service_increment ||
      data.item != this.current_si_uid
    ) {
      chkFbkSlide = true;
